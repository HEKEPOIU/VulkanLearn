...
private:
  ...
  void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS) {
      throw std::runtime_error("failded to record command buffer");
    }
  }
  ...
  void createCommandPool() {
    ...
    poolInfo.queueFamilyIndex = queueFamilyIndices.graphicsFamliy.value();
  ...
  void createGraphicsPipeline() {
    auto vertShaderCode = readFile(Root_DIR "shaders/vert.spv");
    auto fragShaderCode = readFile(Root_DIR "shaders/frag.spv");
  ...
  void drawFrame() {
    ...
    VkSemaphore waitSemphore[] = {imageAvailableSemaphore};
    VkPipelineStageFlags waitStages[] = {
        VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT};
    submitInfo.waitSemaphoreCount = 1;
    submitInfo.pWaitSemaphores = waitSemphore;
